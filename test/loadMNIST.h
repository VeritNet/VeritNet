...
vector<vector<float>> train_image;
vector<vector<float>> train_label;
...
inline void read_Mnist_Label(char filename[], vector<int>& labels)
{
...
inline void convert_array_image(vector< vector<int> >& feature_vector)
{
    for (int i = 0; i < feature_vector.size(); i++)
    {
        for (int a = 0; a < 28; a++)
        {
            train_image.push_back({});
            for (int b = 0; b < 28; b++)
            {
                train_image[i].push_back(feature_vector[i][a * 28 + b] / static_cast<float>(255));
            }
        }
    }
}
